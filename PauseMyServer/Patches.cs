...
namespace PauseMyServer
{
    ...
    // server has confirmed and the menu is still open; closing the menu resumes locally at once
    ...
    internal static class Game_IsPaused_Patch
    {
        ...
        {
            if (!__result && PauseSync.ClientPaused && PauseSync.WantPause) __result = true;
        }
    }

    // ── server: what still runs on a dedicated server while its only client is frozen ──
    //
    // Everything near a player is owned and simulated by that player's client, so a frozen
    // client freezes monsters, ships, physics, smelters and the rest. Three things run on the
