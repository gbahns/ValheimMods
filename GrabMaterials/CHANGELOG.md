...
- Optional **pause while the `/inventory` panel is open**, toggled from a pause button in the panel's top-right corner and remembered between sessions. It goes through the game's own pause, so it works by itself solo or hosting alone; on a dedicated server it takes the Pause My Server mod. The button shows the real pause state: grey when off, orange when the game is genuinely paused, and struck through in red when the pause was requested and refused, so it never implies a pause that is not happening.
