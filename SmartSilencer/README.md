...
- **The console** (F5): `silencer` shows the state, `silencer on` and `silencer off` flip the master switch, `silencer music off` (or `video`, `other`, `focus`) flips one trigger, and `silencer apps` lists every program that has an audio session right now, with its level and how Smart Silencer classifies it. That last one is how to find the name to put in a list when something is not detected, or is detected and should not be.
