...
# Output: OneClickLaunch-<version>.zip in the project directory.
#
# Usage:
...
$zipPath    = Join-Path $projectDir "OneClickLaunch-$Version.zip"
...
dotnet build "$projectDir\OneClickLaunch.csproj" -c Release
...
Add-ZipEntry $zip "$projectDir\bin\Release\net48\OneClickLaunch.dll" "BepInEx/plugins/OneClickLaunch.dll"

$zip.Dispose()
$stream.Dispose()

Write-Host "Package ready: OneClickLaunch-$Version.zip"
