...
public class VersionCheck
{
	...
	private static void ShowConnectionError(FejdStartup __instance)
	{
		...
		// Access m_connectionFailedError (TMP_Text) via reflection to avoid a compile-time
		// dependency on Unity.TextMeshPro → UnityEngine.UI.MaskableGraphic chain.
		var errorField = AccessTools.Field(typeof(FejdStartup), "m_connectionFailedError");
		object? errorComp = errorField?.GetValue(__instance);
		if (errorComp == null) return;
		var textProp = errorComp.GetType().GetProperty("text");
		...
		if (failedCheck)
		{
			...
			errorComp.GetType().GetMethod("ForceMeshUpdate", System.Type.EmptyTypes)?.Invoke(errorComp, null);
			float newHeight = (float)(errorComp.GetType().GetProperty("renderedHeight")?.GetValue(errorComp) ?? 0f) + 105;
