...
# Output: TheObituaries-<version>.zip in the project directory.
#
# Usage:
#   package.ps1 -Version "0.1.0"              # build + zip only
#   package.ps1 -Version "0.1.0" -Publish     # build + zip + upload to Thunderstore
...
$zipPath    = Join-Path $projectDir "TheObituaries-$Version.zip"
...
dotnet build "$projectDir\TheObituaries.csproj" -c Release
...
Add-ZipEntry $zip "$projectDir\bin\Release\net48\TheObituaries.dll" "BepInEx/plugins/TheObituaries.dll"

$zip.Dispose()
$stream.Dispose()

Write-Host "Package ready: TheObituaries-$Version.zip"
